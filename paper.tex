...
\fixme{61}{Write abstract}

\section*{Author Summary}

\fixme{62}{Write author summary}
...
Software Carpentry was founded in 1998 with the goal of 
``making scientists more productive, and their work more
reliable, by teaching them basic computing skills.'' 
They do so through short one or two day intensive workshops that
cover the basics of scientific computing. Programming Historian 
was founded in 2008, and has evolved to become a collaboratively edited
site providing lessons to humanists. 

In this article, we strive for an open-source software model, 
by which we mean lesson materials that are:
...
\item
  Continually maintained, refined, and 
  improved by a community of contributors

\end{enumerate}

To our knowledge, Software Carpentry (and its related
organizations such as Data Carpentry) are the only real-world
example of an organization satisfying these criteria. Programming
Historian, and other open educational resources, satisfy the
first criteria but not necessarily the latter. While they 
might experience a kind of iterative development where the 
lessons are periodically updated by a small author group
(e.g. this happens when a new edition of an academic 
textbook is released), but this is not the same as regular, 
small releases from a large community of contributors. 

Meeting both criteria is critical for two reasons. We need
lessons that are up-to-date with the latest information,
which is crucial given the speed of software development. A lag
between editions means broken dependencies and bugs. Second, lessons
need to continue to live after a key author moves on from 
being able to maintain the lesson. 
